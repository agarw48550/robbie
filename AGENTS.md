...
- Prefer incremental architecture improvements that extend existing modules; avoid large rewrites and preserve behaviour after every change.
- Do not migrate to WebSockets until HTTP is wrapped behind a Transport interface (`connect` / `disconnect` / `send`); WebSocket should be a second implementation so switching is configuration-only.

## Learned Workspace Facts

- Robbie is a desk-pet robot: Raspberry Pi Zero W (Python 3.11+, asyncio) is the AI brain (Gemini Live, planning, memory); UNIHIKER K10 (ESP32-S3) over WiFi is the body (motion, expression/screen, LEDs, speech playback).
...
- Event-driven OS layer (architecture): `EventBus` (`src/events.py`), `RobotState` (`src/robot_state.py`), `EmotionEngine`, `actions`, `Scheduler`, `CommandBus`, `BodyController` — initialised in `src/app.py` before Live AI; body commands go BodyController → CommandBus → `Transport` (HTTP default; WebSocket opt-in via `bridge_transport`) with the same ESP32 JSON.
...
- K10 firmware today is an HTTP server for motion/expression/audio playback; mic capture, wake word, and WebSocket client are not on the body yet (still Pi-side).
